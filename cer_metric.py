"""
Character Error Rate (CER) metric for FiftyOne.

Traditional OCR metric measuring character-level accuracy.
Useful for comparison with classical OCR systems and
character-critical applications.
"""
...
class CERMetric(foo.EvaluationMetric):
    """
    Computes Character Error Rate (CER) for text evaluation.
    
    CER measures the character-level edit distance normalized by
    ground truth length. Lower values are better (0.0 = perfect).
    ...
    def config(self):
        return foo.EvaluationMetricConfig(
            ...
            description="Character-level edit distance (traditional OCR metric)",
            aggregate_key="mean_cer",
            lower_is_better=True,  # Lower CER is better
        )
    ...
    def compute(self, samples, results, case_sensitive=True):
        """
        Compute CER for each sample and return mean.
        ...
        for gt, pred in zip(gt_strings, pred_strings):
            if gt is None or pred is None:
                ...
                score = character_error_rate(gt_text, pred_text)
                scores.append(score)
        
        # Store per-sample CER values
